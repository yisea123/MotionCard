...
int BspSegment(int num, Pose_t* points, KeyPointInf_t* keyPoint,int lineMode)
{
	//系数矩阵对角列
	float* a = NULL;
	...
	for(int i = 0;i < num - 4;i++)
	{
		...
		//判断四点共线（没有考虑零向量情况 fix me）
		if(fabsf(x2*y1-y2*x1)<0.0001f&&fabsf(x3*y1-y3*x1)<0.0001f)
		{
