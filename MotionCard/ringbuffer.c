...
//KeyPointInf_t *fromSZToTZ1RingBuffer;
//KeyPointInf_t *fromSZToHandover1RingBuffer;
//KeyPointInf_t *fromSZToHandover2RingBuffer;
//KeyPointInf_t *fromTZ1ToTZ2RingBuffer;
//KeyPointInf_t *fromTZ1ToHandover2RingBuffer;
//KeyPointInf_t *fromTZ2ToTZ1RingBuffer;
//KeyPointInf_t *fromTZ2ToHandover2RingBuffer;
//KeyPointInf_t *fromHandover1ToTZ1RingBuffer;
//KeyPointInf_t *fromHandover1ToTZ2RingBuffer;
//KeyPointInf_t *fromHandover2ToTZ1RingBuffer;
//KeyPointInf_t *fromHandover2ToTZ2RingBuffer;
//KeyPointInf_t *fromHandover2ToTZ3RingBuffer;
...
//传入点信息，进行规划。并放入缓存池
int InputPoints2RingBuffer(Pose_t *points,int num,int lineMode , velPlan_t velPlan)
{
	int n = 0;
	//将传入的点绘制为B样条，分成15cm一段
	n = BspSegment(num,points,GetRingBufferAdress(),lineMode);
	//dotNumAcquire(n);
	//记录填入的关键点个数
	SetUpPointer(n);
	...
	if(SEND)
	{
		for (int i = 0; i < n; i++)
		{
			...
			(uint8_t *)"PL %d ",(int)ringBuffer[i].point.x);
			USARTDMAOUT(DEBUG_USART,DebugUSARTSendBuf,&DebugUSARTSendBuffCnt,DebugUSARTDMASendBuf,DEBUG_USART_SEND_BUF_CAPACITY,\
			(uint8_t *)"%d ",(int)ringBuffer[i].point.y);
			USARTDMAOUT(DEBUG_USART,DebugUSARTSendBuf,&DebugUSARTSendBuffCnt,DebugUSARTDMASendBuf,DEBUG_USART_SEND_BUF_CAPACITY,\
			(uint8_t *)"%d ",(int)ringBuffer[i].angle);

			USARTDMAOUT(DEBUG_USART,DebugUSARTSendBuf,&DebugUSARTSendBuffCnt,DebugUSARTDMASendBuf,DEBUG_USART_SEND_BUF_CAPACITY,\
			(uint8_t *)"%d ",(int)ringBuffer[i].curvatureR);			
			USARTDMAOUT(DEBUG_USART,DebugUSARTSendBuf,&DebugUSARTSendBuffCnt,DebugUSARTDMASendBuf,DEBUG_USART_SEND_BUF_CAPACITY,\
			(uint8_t *)"%d ",(int)ringBuffer[i].length);
			USARTDMAOUT(DEBUG_USART,DebugUSARTSendBuf,&DebugUSARTSendBuffCnt,DebugUSARTDMASendBuf,DEBUG_USART_SEND_BUF_CAPACITY,\
			(uint8_t *)"%d ",(int)ringBuffer[i].poseAngle);
			USARTDMAOUT(DEBUG_USART,DebugUSARTSendBuf,&DebugUSARTSendBuffCnt,DebugUSARTDMASendBuf,DEBUG_USART_SEND_BUF_CAPACITY,\
			(uint8_t *)"%d ",(int)ringBuffer[i].vellMax);
			USARTDMAOUT(DEBUG_USART,DebugUSARTSendBuf,&DebugUSARTSendBuffCnt,DebugUSARTDMASendBuf,DEBUG_USART_SEND_BUF_CAPACITY,\
			(uint8_t *)"\r\n");
		}
		USARTDMAOUT(DEBUG_USART,DebugUSARTSendBuf,&DebugUSARTSendBuffCnt,DebugUSARTDMASendBuf,DEBUG_USART_SEND_BUF_CAPACITY,\
		(uint8_t *)"%d %d ",(int)GetRingBufferPoint(n).x,(int)GetRingBufferPoint(n).y);
		USARTDMAOUT(DEBUG_USART,DebugUSARTSendBuf,&DebugUSARTSendBuffCnt,DebugUSARTDMASendBuf,DEBUG_USART_SEND_BUF_CAPACITY,\
		(uint8_t *)"\r\n");
	}
	return 1;
